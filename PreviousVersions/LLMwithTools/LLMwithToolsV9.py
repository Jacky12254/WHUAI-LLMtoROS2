...
class GuestInfo(BaseModel):
    ...
    take_order: str = Field(description="客人下单的菜品")

@tool(args_schema=GuestInfo)
def RemenberTool(name: str, take_order: str) -> str:
    """用于记忆或查询客人信息的工具。请根据客人输入调用。"""
    # 模拟记忆逻辑
    return f"已记录：{name}点了{take_order}"
...
# ... [保留原有的 ros_server_worker 启动代码] ...
    ...
    def on_wave_detected(distance=None, person_3d=None):
        ...
        try:
            response = requests.post(url, json=payload, timeout=2.0)
            print_info(ROSInfo, f"挥手发送给 ROS 成功: {response.json()}")
        except Exception as e:
...
1. Detect a waving or calling customer (using VisionControlTool).
...
- Vision: When waiting for a customer, use VisionControlTool(task="hand") to detect waving. Use task="face" to maintain eye contact when talking. Use task="object" when finding items at the bar.
- Memory: When a customer gives an order, immediately use RemenberTool to save their name/table and the specific food/drink items.
- Movement & Actions: You MUST use Ros2ControlTool to move or interact. 
  - To move to a customer: Ros2ControlTool(name="nav_to_customer", flag="start")
  - To move to the bar: Ros2ControlTool(name="nav_to_bar", flag="start")
  - To pick up items: Ros2ControlTool(name="pick_items", flag="start")
  - To place items on the table: Ros2ControlTool(name="place_items", flag="start")
