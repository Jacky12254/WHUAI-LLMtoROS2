...
class LLMwithTools:
    def __init__(self):
        ...
        # config_path = model_path + ".json"  # 对应的 json 配置文件路径
        # print("正在加载高保真离线 TTS 模型...")
        # with open(config_path, 'r', encoding='utf-8') as f:
        #     config = json.load(f)
        # self.voice = PiperVoice(model_path, config)

        # 👇 【新增】：在这里只初始化一次 Pygame，并自动对齐模型的采样率
        # sample_rate = self.voice.config["audio"]["sample_rate"]
    ...
    # def speakout(self, response): 
        ...
        # 例如，使用 pyttsx3 库：
        # self.engine.save_to_file(response, self.output)
        # self.engine.runAndWait()
        # 创建一个标准的 wav 音频文件并写入生成的神经网络声音
    # def speakout(self, response): 
    #     if not response or not response.strip(): 
    #         return False
            
    #     clean_text = re.sub(r'[^\w\s.,!?\'"-]', '', response).strip()
    #     print_info(CodeInfo, f"准备合成文本: {clean_text}")
            
    #     try:
    #         # 获取模型预设的采样率
    #         sample_rate = self.voice.config["audio"]["sample_rate"]
            
    #         with wave.open(self.output, 'wb') as wav_file:
    #             # 必须严格按照这个顺序设置参数
    #             wav_file.setnchannels(1)       # 单声道
    #             wav_file.setsampwidth(2)       # 16-bit PCM (2 bytes)
    #             wav_file.setframerate(sample_rate) 
                
    #             # 核心修正：确保 synthesize 接收的是已经打开并配置好的 wav_file 对象
    ...
    #     except Exception as e:
    #         print_info(CodeInfo, f"❌ TTS 写入异常: {e}")
    #         return False
    import subprocess # 确保文件顶部有导入这个包
    ...
    def speakout(self, response): 
        ...
        try:
            ...
            # 【终极修复方案】：通过 stdin 安全管道将文本传给 piper CLI
            # 这样不用 shell=True，免疫特殊符号报错，稳定生成完整的 WAV
