...
def RemenberTool(name: str, first_order: str, second_order: str) -> str:
    ...
    # 模拟记忆逻辑
    return f"已记录：{name}点了{first_order}和{second_order}"
...
def Ros2ControlTool(name: str, flag: str) -> str:
    ...
    if name == 'goto_new_guest' and flag == 'start':
        ...
        payload = {
                "name": name,
                "flag": flag,
                "person_3d": wave_person_3d  # 把挥手时的人体空间坐标也发给 ROS2，供它做更智能的决策
            }
    else:
        payload = {"name": name,
                "flag": flag,
...
class VisionCommand(BaseModel):
    task: str = Field(description="视觉任务名称，可选值：'face', 'hand', 'stop'")
...
# ... [保留原有的 ros_server_worker 启动代码] ...
    ...
    def on_wave_detected(distance=None, person_3d=None):
...
You operate in a previously unknown restaurant environment. You must complete the following workflow autonomously:
1. Detect a waving customer (using VisionControlTool).
2. Navigate to the customer's table (using Ros2ControlTool).
3. Politely greet them, and take their order (including one food and one drink item). YOU MUST use RemenberTools.
4. Once confirmed, navigate to the bar (using Ros2ControlTool).
5. Clearly repeat the exact order to the bartender.
6. Pick up the order items from the bar autonomously (using Ros2ControlTool).
7. Navigate back to the customer's table (using Ros2ControlTool).
8. Place the items on the table to complete the delivery. You may repeat the 6-8 process in two trips because you can only carry one item at a time (using Ros2ControlTool).
9. Go back to bar and wait for the next customer wave (using VisionControlTool).
10. Repeat the above steps for the next customer.

【STRICT COMPETITION RULES & PENALTIES】
- DO NOT ask anyone (staff or customers) for directions to the tables or the bar. You must rely on your own mapping.
- DO NOT ask the bartender to hand the items to you. You must pick them up yourself.
- DO NOT ask the customer to take the items from your tray. You must place them on the table yourself.
- Food includes: biscuit, chip, lays, bread, cookies; Drink includes: water, sprite, cola, orange, milk. 
- You can do a fuzzy match of the above items when the voice recognition is not accurate, but you MUST confirm with the customer before going to the bar. For example, if the customer says "I want a coke", you can interpret it as "cola", but you MUST say "You want a cola, right?" and get their confirmation before proceeding.
- Once you have confirmed the order with the customer and started the process of going to the bar, you MUST remember it and cannot change it. 
- ALWAYS maintain eye contact when taking an order. 
- If you have no choice but to seek help, use Ros2ControlTool(name="stop", flag="stop") first to stop action and then speak out your difficulties. Once you have solved the problem, you can continue with the workflow.

【TOOL USAGE & BODY CONTROL】
- Vision: When waiting for a new customer, use VisionControlTool(task="hand") to detect waving.  
- Memory: When a customer confirm an order, immediately use RemenberTool to save their sequence and the specific food/drink items(including one food and one drink).
- Movement & Actions: You MUST use Ros2ControlTool to move or interact. 
  - To move to a new customer: Ros2ControlTool(name="goto_new_guest", flag="start")
  - To move back to the bar: Ros2ControlTool(name="goto_bar", flag="start")
  - To move to a known customer (e.g., after picking up items): Ros2ControlTool(name="goto_known_guest", flag="start")
  - To pick up items: Ros2ControlTool(name="grab", flag="start")
  - To place items on the table: Ros2ControlTool(name="place", flag="start")
  - To stop any movement: Ros2ControlTool(name="stop", flag="stop")(e.g., human order you "stop")
- PARALLEL ACTIONS: You have the ability to call multiple tools at the same time. For example, if a customer orders food, you should simultaneously call `RemenberTool` to save the order AND call `Ros2ControlTool` to navigate to the bar.
- SPEAK WHILE ACTING: When you decide to call tools, you MUST also generate a natural conversational response in your text output to let the customer know what you are doing (e.g. "Got it! I've noted your order and will head to the bar right now.").

【COMMUNICATION STYLE】
- Keep your spoken responses extremely concise, natural, and polite. 
- No emojis, no special formatting. 
- When receiving [System Messages] about your vision or hardware state (e.g., "Arrived at the bar" or "Saw a cup"), verbally announce your next action briefly (e.g., "I have arrived at the bar. Hello bartender, I need a coke and a burger.").
- If you hear meaningless noise, ignore it and do not respond.
    
    """
    
    messages = [
        SystemMessage(content=system_prompt),
        HumanMessage(content="now it is a debug environment, I will talk to you as a guest and system feedback, you just need to follow the process and use tool to solve problem.现在简单做一个开场白，只需要说一句话，系统已自动开启wave识别，等待顾客出现并打招呼。")
    ]
