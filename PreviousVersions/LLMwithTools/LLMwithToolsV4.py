...
from track.yolov8_deepsort_tracking.track2 import track_person_loop,recognize_people_set_target

# --- 人脸识别回调 ---
last_face_time = {} # 记录每个人上次被打招呼的时间
def on_face_detected(name):
    current_time = time.time()
    # 如果是 unknown，或者15秒内刚打过招呼，就不理他
    if name == "unknown" or (current_time - last_face_time.get(name, 0) < 15.0):
        return
...
def on_object_detected(obj_name, coordinates):
    ...
    current_time = time.time()
    # 8秒冷却时间
    if current_time - last_obj_time < 8.0: return
    if is_speaking.is_set(): return
...
# --- 人物跟踪回调 ---
last_track_time = 0
def on_person_tracked(distance, coordinates):
    global last_track_time
    current_time = time.time()
    # 10秒播报一次跟踪状态
    if current_time - last_track_time < 10.0: return
    if is_speaking.is_set(): return
    
    last_track_time = current_time
    info = f"正在跟踪目标人物，目前距离 {distance:.2f} 米，坐标 {coordinates}。"
    event_queue.put({"source": "vision_system", "content": info})

def run_person_track_worker():
    track_person_loop(stop_event=vision_stop_event, callback=on_person_tracked)
...
class VisionCommand(BaseModel):
    task: str = Field(description="视觉任务名称，可选值：'face' (人脸识别), 'object' (物体检测), 'track' (人物跟踪), 'hand' (手势识别), 'stop' (停止视觉)")

@tool(args_schema=VisionCommand)
def VisionControlTool(task: str) -> str:
    """当客人要求你认人、看物体、跟随/跟踪某人、或者识别手势时，必须调用此工具开启相应的视觉摄像头任务。"""
    global current_vision_task
    
    if task == current_vision_task:
        return f"视觉任务 {task} 已经在运行中了。"

    # 1. 告诉旧的视觉线程：请立刻退出并释放摄像头！
    print(f"🛑 正在停止旧的视觉任务: {current_vision_task}...")
    vision_stop_event.set()
    time.sleep(1.5)  # 等待底层释放摄像头 (RealSense的pipeline停止需要一点时间)
    vision_stop_event.clear() # 重置开关
    
    current_vision_task = task
    
    # 2. 根据大模型的指令，开启新的子线程
    if task == 'face':
        threading.Thread(target=run_face_detect_worker, daemon=True).start()
        return "已开启人脸识别模式，正在扫描..."
    elif task == 'object':
        threading.Thread(target=run_object_detect_worker, daemon=True).start()
        return "已开启物体检测模式，正在寻找物体..."
    elif task == 'track':
        threading.Thread(target=run_person_track_worker, daemon=True).start()
        return "已开启人物跟踪模式..."
    elif task == 'hand':
        threading.Thread(target=vision_listener_worker, daemon=True).start() # 你之前的 hand2 线程
        return "已开启手势识别模式..."
    elif task == 'stop':
...
def is_meaningful(text):
    ...
    useless_words = ["Yeah.", "What", "Okay.", "the.", "um", "uh", "hello.", "hi.", "test"]
...
if __name__ == "__main__":
    ...
    def voice_listener_worker():
        ...
        while True:
            # 【核心逻辑】：如果大模型正在处理任务，或者正在说话，这里会完全阻塞（挂起），
            # 不消耗 CPU，也不会去调用 get_next_utterance() 打开麦克风！
            # 只有当 is_processing 是 clear 状态时，才会放行。
            if is_processing.is_set():
                time.sleep(0.1) # 稍微睡一下，防止死循环
                continue
            ...
            # 刚说完的1.5秒内，丢弃残音
            if time.time() - last_speak_end_time < 3.5:
                continue
...
【重要规则】
    ...
    【视觉与感知能力】
    当客人要求你寻找某人、看某物、或者跟踪客人(follow someone)时，你必须调用 VisionControlTool 开启对应的摄像头模式。
    模式包括：'face' (认人), 'object' (看东西), 'track' (跟随), 'hand' (看手势), 'stop' (关闭眼睛)。
...
# === 5. 主循环：处理事件队列 ===
    ...
    try:
        while True:
            ...
            # 收到任何需要处理的事件，立刻“上锁”！这会阻止语音线程下一轮打开麦克风
            is_processing.set() 
            ...
            try:
                ...
                # 【关键收尾】：不管上面执行成功还是报错，最后一定要“解锁”
                # 解锁后，voice_listener_worker 才会继续执行，重新打开麦克风
                print("\n[系统空闲] 重新开启麦克风环境监听...")
                is_processing.clear()
