...
6. After saving memory successfully, autonomously use Ros2ControlTool(name="goto_bar", flag="start") to go to the bar.
7. Speak to the bartender to place the order, pick up items using Ros2ControlTool(name="grab", flag="start").
8. Deliver the items by returning to the customer using Ros2ControlTool(name="goto_known_guest", flag="start") and placing them using Ros2ControlTool(name="place", flag="start"). Repeat grab-and-place trips if needed.
...
--- Order 2 ---
10. Upon arriving at the bar after finishing Order 1, you MUST autonomously trigger VisionControlTool(task="hand") to start wave detection again.
...
- NEVER use RemenberTool before explicit verbal confirmation.
    ...
    messages = [
        ...
        HumanMessage(content="now it is a debug environment, I will talk to you as a guest and system feedback, you just need to follow the process and use tool to solve problem,you don't need to care whether tool is available or not.现在简单做一个开场白，只需要说一句话，系统已自动开启wave识别，等待顾客出现并打招呼。")
    ]
