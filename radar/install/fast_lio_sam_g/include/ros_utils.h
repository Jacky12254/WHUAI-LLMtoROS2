...
inline ros::Time offset_ros_time(const ros::Time &stamp, double offset_sec)
{
    return stamp + ros::Duration(offset_sec);
}
#elif defined(USE_ROS2)
...
inline rclcpp::Time offset_ros_time(const rclcpp::Time &stamp, double offset_sec)
{
    return stamp + rclcpp::Duration::from_seconds(offset_sec);
}
