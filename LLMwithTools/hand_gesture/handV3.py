...
# ---------------------------- 主程序 ----------------------------
def start_gesture_recognition(on_wave_callback=None, on_point_callback=None, stop_event=None):
                ...
                if len(frame_buffer) == 3:
                    ...
                    if wave_dir:
                        ...
                        if (now - last_wave_time >= cooling_time) or (wave_dir != last_wave_dir):
                            print(f"挥手动作：{wave_dir}, 距离：{current_distance:.2f}米")
                            # ==========================================
                            # 【核心修改点】：回调函数增加 distance 参数
                            if on_wave_callback:
                                on_wave_callback(wave_dir, current_distance)
                            # ==========================================
                            last_wave_time = now
