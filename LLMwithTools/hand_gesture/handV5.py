...
def start_gesture_recognition(on_wave_callback=None, 
    ...
    last_point_time = 0
    wrist_3d_buffer = deque(maxlen=5)
                        ...
                        if len(wave_history) >= 2:
                            if (now - last_wave_time >= cooling_time):
                                ...
                                if wrist_dist > 0:
                                    ...
                                    wrist_3d_buffer.append(person_3d)

                                # 只要队列里有数据，我们就算平均值（不需要死板地等满 5 个）
                                if len(wrist_3d_buffer) > 0:
                                    px = sum(p[0] for p in wrist_3d_buffer) / len(wrist_3d_buffer)
                                    py = sum(p[1] for p in wrist_3d_buffer) / len(wrist_3d_buffer)
                                    pz = sum(p[2] for p in wrist_3d_buffer) / len(wrist_3d_buffer)
                                    
                                    avg_person_3d = (px, py, pz)
                                    print(f"👋 确认有效挥手！动作: {[w[1] for w in wave_history]}, 客人三维坐标: ({px:.2f}, {py:.2f}, {pz:.2f})")
                                    
                                    # 触发回调给 LLM，将精确三维坐标一起传出去
                                    if on_wave_callback:
                                        on_wave_callback(wave_dir, wrist_dist, avg_person_3d)
                                else:
