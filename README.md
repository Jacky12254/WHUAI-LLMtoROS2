...
## 代码工作流
    ...
    语音识别/手势识别/物品识别 --->  结果传入大模型 
    
    --->  大模型发出调用工具指令(支持多个工具同时调用）并作出回答 --->  工具内部方法实现 
    
    ---http传输--->  ROS2Link节点对指令解析  --->  ROS2发布话题  
    
    --->  机器人完成操作并反馈  --->  ROS2Link节点订阅话题并传入callback函数进行解析 
    
    ---http传输--->  LLM相关代码的解析，并传入大模型 
    
    --->  大模型作出新回答与新调用 --->  语音播报/工具调用
...
特别感谢其他贡献者，他们都是WHURobocup-Home的成员，他们在ROS2支持方面相当给力，并且也给予了这条技术路线极大的支持。

衷心感谢Gemini,Qwen,deepseek,CodeX等一众大模型的鼎力相助。
