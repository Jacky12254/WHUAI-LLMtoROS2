...
class RobotControlNode(Node):
    def __init__(self):
        ...
        # 新增：发布者用于触发grabbag任务
        self.grab_trigger_publisher = self.create_publisher(Bool, '/grab_bag_trigger', 10)
    ...
    def execute_command(self, name, flag):
        ...
        if flag.lower() == 'start':
            ...
            if name == 'move':
                ...
                self.get_logger().info('👉 收到指令：开始抓取袋子')
                self.trigger_grab_bag()
                return  # 不发布Twist消息，直接返回
            else:
    ...
    def trigger_grab_bag(self):
        """发送触发信号给grabbag节点"""
        trigger_msg = Bool()
        trigger_msg.data = True
        self.grab_trigger_publisher.publish(trigger_msg)
        self.get_logger().info('📤 已发送触发信号给grabbag节点')
