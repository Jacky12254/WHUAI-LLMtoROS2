...
class RobotControlNode(Node):
    def __init__(self):
        ...
        # === 人物跟踪/跟随相关状态 ===
        self.is_following = False           # 是否正在跟随人物
...
# ============================== 处理人物跟踪数据 ==================================
    def process_person_track_data(self, data):
        """
        处理来自 conda 环境的人物跟踪数据，计算并发布 /cmd_vel。
        
        参数 data (dict):
            track_id: 跟踪ID
            x, y, z: 人物在相机坐标系下的3D坐标（米）
        """
        ...
        if abs(distance_error) > 0.1:  # 10cm 死区
            msg.linear.x = self.follow_linear_speed * (1.0 if distance_error > 0 else -0.5)
            # 前进速度 0.3，后退速度 0.15
        else:
            msg.linear.x = 0.0  # 到达目标距离
        
        # 角度控制：转向正对目标
        if abs(angle_to_target) > 0.05:  # ~3度死区
        ...
        if int(time.time()) % 5 == 0:
            self.get_logger().info(
                f'llmbridge: 🎯 跟随 TrackID={track_id} '
                f'距离={depth:.2f}m 偏移={horizontal_offset:.2f}m '
                f'速度=(linear={msg.linear.x:.2f}, angular={msg.angular.z:.2f})'
            )
