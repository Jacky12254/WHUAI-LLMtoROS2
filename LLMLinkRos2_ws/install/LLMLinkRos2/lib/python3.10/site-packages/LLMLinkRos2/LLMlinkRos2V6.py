...
# ============================== 处理大模型指令和反馈 ==================================
    def execute_command(self, name, flag, person_3d=None):
        """处理来自大模型的控制指令"""
        msg = Twist()
        self.current_task = name
        if flag.lower() == 'start':
            if name == 'goto_new_guest':# 前往新客人位置
                self.get_logger().info('llmbridge: 👉 收到指令：开始前往新客人位置')
                msg.linear.x = 0.0
                msg.linear.y = 0.1
                self.is_moving = True
                self.wave_person_3d = person_3d  # 存储挥手目标的3D坐标
                # TODO: 将 wave_person_3d 传递给底盘控制逻辑，进行导航
...
def control_robot():
    ...
    if ros_node and name and flag:
        ...
        return jsonify({
            "status": "success", 
            "msg": f"指令 [{name} - {flag}] 已成功下发至LLMlinkRos2"
        }), 200
    else:
